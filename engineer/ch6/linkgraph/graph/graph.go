...
type Iterator interface {
	...
	// Error returns the last error encountered byt the iterator.
	Error() error
...
type Graph interface {
   ...
   Links(fromID, toID uuid.UUID, retrieveBefore time.Time) (LinkIterator, error)
	...
	// to the [fromID, toID) range and were updated before the provided timestamp.
	Edges(fromID, toID uuid.UUID, updatedBefore time.Time) (EdgeIterator, error)

	// RemoveStableEdges removes any edge that originates from the specified
