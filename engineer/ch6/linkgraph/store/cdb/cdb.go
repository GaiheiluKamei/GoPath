...
import (
	"database/sql"
	"github.com/GaiheiluKamei/books/engineer/ch6/linkgraph/graph"
...
var (
	upsertLinkQuery = `INSERT INTO links (url, retrieved_at) VALUES ($1, $2) ON CONFLICT (url)
DO UPDATE SET retrieved_at =GREATEST(links.retrieved_at, $2) RETURNING id, retrieved_at`
...
ON CONFLICT (src, dst) DO UPDATE SET updated_at=NOW() RETURNING id, updated_at`

	edgeInPartitionQuery = "SELECT id, src, dst, updated_at FROM edges WHERE src >= $1 AND src < $2 AND updated_at < $3"
...
// NewCockroachDbGraph returns a CockroachDbGraph instance that connects to the cockroachdb instance
// specified by dsn.
...
// Edges returns an iterator for the set of edges whose source vertex IDs belong
// to the [fromID, toID) range and were last updated before the provided value.
func (c *CockroachDBGraph) Edges(fromID, toID uuid.UUID, updatedBefore time.Time) (graph.EdgeIterator, error) {
	rows, err := c.db.Query(edgeInPartitionQuery, fromID, toID, updatedBefore.UTC())
	if err != nil {
...
// isForeignKeyViolationError returns true if err indicated a foreign key
